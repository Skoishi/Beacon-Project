...
class Reader:
  ...
  def wf(self,ch = 0, bd = 0):  
    ...
    try:
      ev = self.event() 
      v = numpy.copy(numpy.frombuffer(ev.getData(ch,bd), numpy.dtype('float64'), ev.getBufferLength()))
      return v
    except Exception as e:
      print('\nError in %s'%inspect.stack()[0][3])
      print(e)
  ...
  def readout_time(self):
    ...
    '''
    try:
      return numpy.frombuffer(cppyy.ll.cast['uint32_t*'](self.stat.readout_time), dtype=numpy.uint16, count=60).reshape(3,20)
    except Exception as e:
      print('\nError in %s'%inspect.stack()[0][3])
      print(e)
      exc_type, exc_obj, exc_tb = sys.exc_info()
  ...
  def returnBeamScalers(self, expected_max_beam=19, plot=False):
    ...
    try:
      ...
      while failed == False:
        try:
          if beam_index == 0:
            N = self.status_tree.Draw("beam_scalers[0][%i]/10.:trigger_thresholds[%i]:readout_time"%(beam_index,beam_index),"","goff")
            beam_scalers = numpy.frombuffer(self.status_tree.GetV1(), numpy.dtype('float64'), N)
            trigger_thresholds = numpy.frombuffer(self.status_tree.GetV2(), numpy.dtype('float64'), N)
            readout_time = numpy.frombuffer(self.status_tree.GetV3(), numpy.dtype('float64'), N).astype(int)
          else:
            ...
            trigger_thresholds = numpy.vstack((trigger_thresholds,numpy.frombuffer(self.status_tree.GetV2(), numpy.dtype('float64'), N)))
          if beam_index is not None:
            if beam_index == expected_max_beam:
              failed=True
          beam_index += 1
